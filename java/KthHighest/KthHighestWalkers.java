...
public class KthHighestWalkers {
  public static void main( String [] args ) {
    try {
      ...
      int startRow = 0, nextRow = 0, startCol = 0, currVal = 0, count = 0, idx = 0;
      ...
      for( int i = startRow - 1; i >= 1; i-- ) {
        if( (startCol = getStartColumnForRow(i, startRow )) <= n ) {
          ...
          System.out.println( "Added a walker for row "+ i +", start col: " +startCol+ " count now (" + count + ")");
        } else {
          count += ((n - startRow) * i);
          if( m > startRow ) {
            count += ((m - startRow) * i);
