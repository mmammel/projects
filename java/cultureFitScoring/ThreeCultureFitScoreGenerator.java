...
/**
 ...
 Also, the organization did the same ranking on their own perception, that score is recognized as "org".

 So in the end, each trait has two #'s - a preference and a perception number, such that:
...
public class ThreeCultureFitScoreGenerator {
  ...
  public ThreeCultureFitScoreGenerator() {
  ...
  public double run(boolean quiet) {
    ...
    Map<String,Integer> orgSums = new HashMap<String,Integer>();
    ...
    for( int i = 0; i < PAIRINGS.length; i++ ) {
      ...
      Collections.shuffle( this.prefs, new Random(System.nanoTime()) );
      tempScore = this.prefs.get(0);
      tempSum = orgSums.get(temp[0]);
      orgSums.put(temp[0], (tempSum + tempScore) );
      tempSum = orgSums.get(temp[1]);
      orgSums.put( temp[1], (tempScore - tempSum) );
    }
    ...
    Collections.shuffle( this.ranks, new Random(System.nanoTime()) );
    for( int k = 0; k < VALUES.length; k++ ) {
      tempSum = orgSums.get( VALUES[k] );
      orgSums.put( VALUES[k], tempSum + this.ranks.get(k) );
    }

    // Dump Results
    int prefScore = 0, perceptionScore = 0, orgScore = 0;
    int percentage = 0;
    ...
    for( int i = 0; i < VALUES.length; i++ ) {
      ...
      orgScore = orgSums.get( VALUES[i] );
      // Old way (flat planes)
      //percentage = new Double(100.0 - (new Float(( Math.abs( prefScore - perceptionScore)) / 22.0) * 100.0)).intValue();
      x = new Double( prefScore );
      y = new Double( perceptionScore );
      w = new Double( orgScore );
      percentage = new Double(zachScore(x,y,w)).intValue();
      //percentage = new Double(avgScore(x,y,w)).intValue();
      //percentage = new Double(avgDiffScore(x,y,w)).intValue();
      //percentage = new Double(simpleScore(x,y,w)).intValue();
     
      totalPercentage += percentage;
      if( !quiet ) System.out.println( "[" + VALUES[i] + "] : " + prefScore + "," + perceptionScore + "," + orgScore + " -> " + percentage );
      if( !quiet ) System.out.println( "Diffs [" + VALUES[i] + "] : " + (prefScore-perceptionScore) + "\t" + (prefScore-orgScore) + "\t" + (perceptionScore-orgScore) );
    }
  ...
  public static double zachScore( double x, double y, double w ) {
    double retVal = 0.0;
    double exp = Math.pow((2.0d * (x-y))/22.0d, 2) + Math.pow((2.0d * (w-y))/22.0d, 2) + Math.pow((2.0d * (w-x))/22.0d, 2);
    retVal = 100.0d * Math.pow( Math.E, (-2.0d * exp ) );
    return retVal;
  }

  public static double avgScore( double x, double y, double w ) {
    double total = 0.0;

    double xy = 100.0d * Math.pow( Math.E, (-2.0d * Math.pow((3.0d * (x-y))/22.0d, 2)) );
    double wy = 100.0d * Math.pow( Math.E, (-2.0d * Math.pow((3.0d * (w-y))/22.0d, 2)) );
    double wx = 100.0d * Math.pow( Math.E, (-2.0d * Math.pow((3.0d * (w-x))/22.0d, 2)) );
  ...
  public static double avgDiffScore( double x, double y, double w ) {

    double xy = x - y;
    double wy = w - y;
    double wx = w - x;
    double avg = (xy+wy+wx)/3.0d;

    return 100.0d * Math.pow( Math.E, (-2.0d * Math.pow((3.0d * avg)/22.0d, 2)));
  }

  public static double simpleScore( double x, double y, double w ) {
    double total = 0.0;

    double xy = 100.0d - (Math.abs(x-y)/46.0d)*100.0d;
    double wy = 100.0d - (Math.abs(w-y)/46.0d)*100.0d;
    double wx = 100.0d - (Math.abs(w-x)/46.0d)*100.0d;
