...
int NewtonApple_hull_3D( std::vector<R3> &pts, std::vector<Tri> &hulk)
{
  ...
  // does sorting by z(x(y)) using the overloaded
  // < operator.
  sort( pts.begin(), pts.end() );
...
int init_hull3D( std::vector<R3> &pts, std::vector<Tri> &hull)
{
  ...
  Tri T1(0,1,2);
  float r0 = pts[0].r, c0 = pts[0].c, z0 = pts[0].z; // Three points: (r0,c0,z0) (r1,c1,z1) (r2,c2,z2)
  float r1 = pts[1].r, c1 = pts[1].c, z1 = pts[1].z;
  float r2 = pts[2].r, c2 = pts[2].c, z2 = pts[2].z;
  
  Mr = r0+r1+r2; // M* sums of the components
  Mc = c0+c1+c2; // the big M vector is the sum of all three vectors.
  Mz = z0+z1+z2;
  ...
  int numt = 2; // number of triangles, I presume.

  for( int p=3; p<nump; p++){ // add points until a non-coplanar set of points is achieved.
    R3 &pt = pts[p];

    Mr += pt.r; mr = Mr/(p+1); // Keep adding onto the sum vector... 
    Mc += pt.c; mc = Mc/(p+1); // the little m vector is the "average" vector, or origin.
    Mz += pt.z; mz = Mz/(p+1);
    ...
    float z = pt.z;         // 
    xlist.clear();          // empty the "x list", not sure what that is yet.
    ...
    for( int h=numh-1; h>=0; h--){  // Walk backwards through the existing Tri's
      ...
      float dr = r-R1;  //
      float dc = c-C1;  // (dr,dc,dz) is this point minus the first point in the current Tri
      float dz = z-Z1;  //

      float d = dr*t.er + dc*t.ec + dz*t.ez; // dot product with the triangle's normal vector

      if( d > 0 ){              // dot product is greater than 0
	    hvis = h;               // Note which Tri it was (index into hull)
	    hull[h].keep = 0;       // Don't keep this hull?
	    xlist.push_back(hvis);  // this hull is on the x-list.
